...
void wall_follow_pid() {
    ...
    if (left_distance == 0xFFFF) left_distance = max_sensor_distance;
    if (right_distance == 0xFFFF) right_distance = max_sensor_distance;
    
    // Filter out readings beyond reliable range
    ...
    switch (wall_follow_mode) {
        ...
        case WALL_FOLLOW_CENTER:
            ...
            // Positive error means we're closer to left wall, need to move right
            // Negative error means we're closer to right wall, need to move left
    ...
    // For centering: positive PID output means move right (left motor faster)
    // For centering: negative PID output means move left (right motor faster)
...
// Arduino-style driving control with automatic wall centering
void driving_control(float currentAngle, float gyroX) {
    int16_t deltaAngle = (int16_t)(targetAngle - currentAngle);
    
    // Always use PID wall following for automatic centering during forward movement
    if (wall_follow_mode == WALL_FOLLOW_CENTER) {
        // Auto-center between walls using PID
        wall_follow_pid();
    } else if (abs(deltaAngle) < 5) {
        // Use PID for specific wall following when angle is close
        wall_follow_pid();
    } else {
        // Use original angle-based steering for larger angle corrections
        motor_forward();
...
int main() {
    ...
    serial_string("'W' - Drive forward with AUTO-CENTERING between walls\n");
    ...
    while (1) {
        ...
        if (serial_available()) {
            ...
            if (cmd == 'W' || cmd == 'w') { // drive forward with auto-centering
                serial_string("forward with auto-centering\n");
                isDriving = 1;
                wall_follow_mode = WALL_FOLLOW_CENTER; // Automatically center between walls
