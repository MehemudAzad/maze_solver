...
#define FRONT2_OBSTACLE_THRESHOLD_CM 18 // Distance in cm to trigger left turn
#define WALL_DISTANCE_THRESHOLD_CM 30  // Distance threshold for wall detection (left-hand rule)
#define FORWARD_MOVE_DURATION_MS 1100   // Duration to move forward after turns

// PID constants for wall centering
#define CENTERING_PID_KP 4.0      // Proportional gain for centering between walls
#define CENTERING_PID_KD 1.5      // Derivative gain for centering
#define MAX_CORRECTION 60         // Maximum speed correction
#define MIN_WALL_DISTANCE 8       // Minimum distance to consider wall valid
...
// PID variables for wall centering
float previous_centering_error = 0.0; // Previous error for derivative calculation
...
void correct_straight_path() {
    ...
    // Priority 1: Turn left if there's enough space on the left (>20cm)
    ...
    if (front_distance > FRONT2_OBSTACLE_THRESHOLD_CM && front_distance != 0xFFFF) {
        // Dynamic wall centering - keep equal distance from both walls
        serial_string("Centering between walls...\n");
        
        // Set motor direction for forward movement
        PORTA &= ~((1 << MOTOR1_IN1) | (1 << MOTOR2_IN1));
        PORTA |= (1 << MOTOR1_IN2) | (1 << MOTOR2_IN2);
        
        float centering_error = 0.0;
        int16_t correction = 0;
        
        // Check if we have valid readings from both sides
        if (left_distance != 0xFFFF && right_distance != 0xFFFF && 
            left_distance > MIN_WALL_DISTANCE && right_distance > MIN_WALL_DISTANCE) {
            
            // Calculate centering error: positive = too close to left, negative = too close to right
            centering_error = (float)left_distance - (float)right_distance;
            
            // Calculate derivative for smoother control
            float derivative = centering_error - previous_centering_error;
            
            // PID control for centering
            correction = (int16_t)(CENTERING_PID_KP * centering_error + CENTERING_PID_KD * derivative);
            
            // Store for next iteration
            previous_centering_error = centering_error;
            
            // Limit correction magnitude
            if (correction > MAX_CORRECTION) correction = MAX_CORRECTION;
            if (correction < -MAX_CORRECTION) correction = -MAX_CORRECTION;
            
            serial_string("Both walls: L=");
            serial_num(left_distance);
            serial_string(", R=");
            serial_num(right_distance);
            serial_string(", CenterErr=");
            serial_num((int16_t)centering_error);
            serial_string(", Corr=");
            serial_num(correction);
            serial_string("\n");
        }
        else if (left_distance != 0xFFFF && left_distance > MIN_WALL_DISTANCE) {
            // Only left wall detected - try to maintain reasonable distance
            float target_left_distance = 15.0; // Fallback target when only one wall
            centering_error = left_distance - target_left_distance;
            correction = -(int16_t)(CENTERING_PID_KP * centering_error * 0.5); // Gentler correction
            
            if (correction > MAX_CORRECTION/2) correction = MAX_CORRECTION/2;
            if (correction < -MAX_CORRECTION/2) correction = -MAX_CORRECTION/2;
            
            serial_string("Left wall only: ");
            serial_num(left_distance);
            serial_string("cm, Corr=");
            serial_num(correction);
            serial_string("\n");
        }
        else if (right_distance != 0xFFFF && right_distance > MIN_WALL_DISTANCE) {
            // Only right wall detected - try to maintain reasonable distance
            float target_right_distance = 15.0; // Fallback target when only one wall
            centering_error = target_right_distance - right_distance;
            correction = (int16_t)(CENTERING_PID_KP * centering_error * 0.5); // Gentler correction
            
            if (correction > MAX_CORRECTION/2) correction = MAX_CORRECTION/2;
            if (correction < -MAX_CORRECTION/2) correction = -MAX_CORRECTION/2;
            
            serial_string("Right wall only: ");
            serial_num(right_distance);
            serial_string("cm, Corr=");
            serial_num(correction);
            serial_string("\n");
            ...
            // No valid walls detected - use gyroscope correction as fallback
            float angle_error = currentAngle - forwardSetpointAngle;
            correction = (int16_t)(Kp * angle_error);
            serial_string("No walls - gyro correction\n");
        }
        
        // Apply correction to motor speeds (positive correction = steer right)
...
void execute_turn_sequence() {
    if (sequenceStep == 0 && !isTurning && !isMovingStraight) {
        ...
        // Brief forward movement after left turn, then reset to normal navigation
        serial_string("Moving forward briefly after turn...\n");
        PORTA &= ~((1 << MOTOR1_IN1) | (1 << MOTOR2_IN1));
        PORTA |= (1 << MOTOR1_IN2) | (1 << MOTOR2_IN2);
        set_right_motor_speed(motor_speed);
        set_left_motor_speed(motor_speed);
        _delay_ms(FORWARD_MOVE_DURATION_MS); // Move forward for 100ms
        motor_stop();
        sequenceStep = 0; // Reset to continuous navigation
        _delay_ms(300); // Brief pause before resuming
    }
        ...
        // Brief forward movement after right turn, then reset to normal navigation
        serial_string("Moving forward briefly after right turn...\n");
        PORTA &= ~((1 << MOTOR1_IN1) | (1 << MOTOR2_IN1));
        PORTA |= (1 << MOTOR1_IN2) | (1 << MOTOR2_IN2);
        set_right_motor_speed(motor_speed);
        set_left_motor_speed(motor_speed);
