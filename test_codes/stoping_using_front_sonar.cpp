...
#define FRONT_OBSTACLE_THRESHOLD_CM 12 // Distance in cm to trigger left turn
...
const float Kp = 0.2;         // Proportional gain for correction (tune if needed)
...
void correct_straight_path() {
    ...
    // Check for obstacle while moving forward using front sonar sensor
    uint16_t front_distance = sonar3_get_distance_cm();
    if (front_distance <= FRONT_OBSTACLE_THRESHOLD_CM && front_distance != 0xFFFF) { // Obstacle detected
        motor_stop();
        isMovingStraight = 0;
        serial_string("Front obstacle detected at ");
        serial_num(front_distance);
        serial_string("cm! Stopping and preparing to turn left...\n");
...
int main() {
  ...
  sonar3_init();  // Front sonar (PB1, PB2)
	...
	serial_string("Front obstacle threshold: ");
	serial_num(FRONT_OBSTACLE_THRESHOLD_CM);
	serial_string("cm\n");
