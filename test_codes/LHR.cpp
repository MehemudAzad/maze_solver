...
uint8_t left_hand_rule_check(void);

// Global variables for left-hand rule maze navigation
...
uint8_t sequenceStep = 0;   // 0=check_left, 1=turn_left, 2=move_forward_brief, 3=normal_forward
uint8_t leftHandRuleActive = 0; // Flag to enable left-hand rule navigation
const int RIGHT_MOTOR_OFFSET = 4; // Speed difference for right motor

// Left-hand rule constants
#define WALL_THRESHOLD 15        // Distance in cm to consider as a wall
#define BRIEF_FORWARD_TIME 2     // 2 * 50ms = 100ms brief forward movement
...
uint16_t brief_forward_counter = 0;    // Counter for brief forward movement
...
// Left-hand rule decision function
uint8_t left_hand_rule_check(void) {
    uint16_t left_distance = sonar2_get_distance_cm();   // Left sensor (sonar2)
    uint16_t right_distance = sonar_get_distance_cm();   // Right sensor (sonar1) 
    uint8_t front_clear = ir_sensor_read();              // Front IR sensor
    
    // Handle sensor errors
    if (left_distance == 0xFFFF) left_distance = 100;  // Assume far if error
    ...
    serial_string(" F:");
    serial_num(front_clear);
    serial_string(" -> ");
    
    // Left-hand rule: Can we go left?
    if (left_distance > WALL_THRESHOLD) {
        serial_string("Can go LEFT\n");
        return 1; // Can turn left
    }
    // Can we go straight?
    else if (front_clear == 1) {
        serial_string("Go STRAIGHT\n");
        return 0; // Go straight
    }
    // Must turn right
    else {
        serial_string("Must go RIGHT\n");
        return 2; // Must turn right
    }
}
...
void execute_left_hand_rule() {
    if (sequenceStep == 0 && !isTurning && !isMovingStraight) {
        // Step 0: Check if we can go left (left-hand rule priority)
        uint8_t decision = left_hand_rule_check();
        
        if (decision == 1) {
            // Can turn left - do it!
            sequenceStep = 1; // Move to left turn step
        } else if (decision == 0) {
            // Go straight - enter normal forward movement
            sequenceStep = 3; // Skip to normal forward movement
            motor_forward_straight();
        } else {
            // Must turn right
            start_right_turn();
            sequenceStep = 0; // After right turn, check again
        }
    }
    else if (sequenceStep == 1 && !isTurning) {
        // Step 1: Execute left turn
        start_left_turn();
        sequenceStep = 2; // Move to brief forward step after turn
    }
    else if (sequenceStep == 2 && !isTurning && !isMovingStraight) {
        // Step 2: Brief forward movement after left turn (100ms)
        serial_string("Brief forward movement after left turn...\n");
        forwardSetpointAngle = currentAngle;
        isMovingStraight = 1;
        brief_forward_counter = 0;
        sequenceStep = 3; // Move to normal forward
    }
    else if (sequenceStep == 3 && isMovingStraight) {
        // Step 3: Normal forward movement with obstacle detection
        if (brief_forward_counter < BRIEF_FORWARD_TIME) {
            // Still in brief forward mode
            brief_forward_counter++;
            correct_straight_path();
        } else {
            // Brief forward complete, check for left again
            sequenceStep = 0; // Go back to checking left
            isMovingStraight = 0;
            motor_stop();
            _delay_ms(100); // Brief pause before next decision
        }
    }
    else if (sequenceStep == 3 && !isMovingStraight) {
        // Normal forward movement - restart if not moving
        motor_forward_straight();
    }
}

void execute_turn_sequence() {
    if (leftHandRuleActive) {
        execute_left_hand_rule();
    } else {
...
int main() {
  ...
  sonar_init();   // Right sensor
  sonar2_init();  // Left sensor
  
	// Initialize LED pin (PB0) as output - but we also use it for IR sensor
	...
	serial_string("Maze Solver with Left-Hand Rule\n");
	serial_string("Press 'C' to calibrate (keep stationary!)\n");
	serial_string("Press 'L' to start LEFT-HAND RULE navigation\n");
	serial_string("Press 'O' to start OBSTACLE AVOIDANCE mode\n");
	serial_string("Press 'S' to STOP navigation\n");
	serial_string("Press 'G' to get sensor data\n");
	...
	while (1) {
		...
		if (serial_available()) {
			...
			if (cmd == 'C' || cmd == 'c') {
				...
				motor_stop();
				leftHandRuleActive = 0; // Stop any navigation;
				...
				serial_string("Ready for navigation. Press 'L' for left-hand rule or 'O' for obstacle avoidance.\n");
				
			} else if (cmd == 'L' || cmd == 'l') {
				if (calibrated) {
					leftHandRuleActive = 1;
					sequenceStep = 0;
					isTurning = 0;
					isMovingStraight = 0;
					brief_forward_counter = 0;
				...
				if (calibrated) {
					leftHandRuleActive = 0;
					sequenceStep = 0;
					isTurning = 0;
					isMovingStraight = 0;
					serial_string("Starting OBSTACLE AVOIDANCE mode!\n");
				} else {
					serial_string("Please calibrate first (press 'C')\n");
				}
			} else if (cmd == 'S' || cmd == 's') {
				...
				isMovingStraight = 0;
				isTurning = 0;
				serial_string("Navigation STOPPED.\n");
			} else if (cmd == 'G' || cmd == 'g') {
				if (calibrated) {
					uint16_t left_dist = sonar2_get_distance_cm();
					uint16_t right_dist = sonar_get_distance_cm();
					uint8_t front_clear = ir_sensor_read();
					
					serial_string("Angle: ");
					serial_num((int16_t)currentAngle);
					serial_string("° | L:");
					serial_num(left_dist == 0xFFFF ? 999 : left_dist);
					serial_string("cm R:");
					serial_num(right_dist == 0xFFFF ? 999 : right_dist);
					serial_string("cm F:");
					serial_num(front_clear);
					serial_string(" | Step:");
